...
Collection schema (:py:class:`schema.SkyCollectionSchema`) are the most important schema class in pyXMIP. These schema connect the
...
most important job of :py:class:`schema.SkyCollectionSchema` instances is to connect the available ``columns`` of the underlying data table
to standardized conventions used by the class for cross matching or other tasks.

Formatting
++++++++++

- ``column_map`` is the most important key of the :py:class:`schema.SkyCollectionSchema`. This specifies the names of special columns in your catalog.
...
- ``default_coord_system`` is the default coordinate system to apply to your data. In general, catalogs may contain more than 1 complete
  coordinate system and so this column is used to determine which frame to use by default. It can be changed on the fly by setting :py:attr:`schema.SkyCollectionSchema.coordinate_system` .

The following is the standard format for the :py:class:`schema.SkyCollectionSchema` class.
